...
final class ProductSyncOutboxDeleteRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        $statement = implode('', array_map(
            ...
            $scope->getType($firstArgument->value)->getConstantStrings(),
        ));

        if (preg_match(self::OUTBOX_DELETE_PATTERN, $statement) !== 1) {
            return [];
        }

        if (preg_match(self::RELEASE_PREDICATE_PATTERN, $statement) === 1) {
            return [];
        }
